...
export default function Admin() {
  ...
  const handleNewPlantSubmit = async (e) => {
    ...
    if (!slug) {
      setFormMessage('Error: Plant name cannot be empty.');
    ...
    try {
      ...
      if (res.ok) {
        setFormMessage('Success: Plant added successfully!');
        setPlantName('');
        setScientificName('');
        ...
        setImageUrl('');
        // Refresh plants list in other tabs if needed
        const p = await (await fetch('/api/plants')).json();
        setPlants(p.plants || []);
      } else {
        const data = await res.json();
        setFormMessage(`Error: ${data.error || 'Failed to add plant.'}`);
      }
    } catch (error) {
      setFormMessage(`Error: ${error.message || 'An unexpected error occurred.'}`);
    } finally {
      setFormSubmitting(false);
    }
  };

  const updateCred = useCallback(async () => {
    setCredMsg('');
    try {
      const u = String(credUser || '').trim();
      const p = String(credPass || '').trim();
      if (u.length < 2) { setCredMsg('Username must be at least 2 characters.'); return; }
      if (p.length < 6) { setCredMsg('Password must be at least 6 characters.'); return; }
      ...
      if (r.ok) {
        setCredPass('');
        setCredMsg('Admin account updated, please log out and log in with the new account.');
      } else {
        const j = await r.json().catch(() => ({}));
        setCredMsg(String(j.error || 'Update failed.'));
      }
    } catch {
      setCredMsg('Update failed.');
    }
  }, [credUser, credPass]);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
        ...
        <button type="button" onClick={() => setActiveTab('add-plant')} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #e5e7eb', background: activeTab==='add-plant' ? '#eef' : '#fff' }}>添加植物</button>
      </div>
      ...
      {activeTab === 'add-plant' && (
        <section style={{ 
          ...
          <form onSubmit={handleNewPlantSubmit} style={{ display: 'grid', gap: '20px', maxWidth: '600px' }}>
            <label style={{ display: 'grid', gap: '8px' }}>
              <span style={{ fontWeight: 600, color: sageGreenDark, fontSize: '15px' }}>Plant Name:</span>
              <input
                type="text"
                value={plantName}
                ...
                onFocus={(e) => { e.currentTarget.style.borderColor = sageGreen; }}
                onBlur={(e) => { e.currentTarget.style.borderColor = warmCreamDark; }}
              />
            </label>
            <label style={{ display: 'grid', gap: '8px' }}>
              <span style={{ fontWeight: 600, color: sageGreenDark, fontSize: '15px' }}>Scientific Name:</span>
              <input
                type="text"
            ...
            <label style={{ display: 'grid', gap: '8px' }}>
              <span style={{ fontWeight: 600, color: sageGreenDark, fontSize: '15px' }}>Toxicity Level:</span>
              <select
                value={toxicityLevel}
            ...
            <label style={{ display: 'grid', gap: '8px' }}>
              <span style={{ fontWeight: 600, color: sageGreenDark, fontSize: '15px' }}>Care Difficulty:</span>
              <select
            ...
            <label style={{ display: 'grid', gap: '8px' }}>
              <span style={{ fontWeight: 600, color: sageGreenDark, fontSize: '15px' }}>English Description:</span>
              <textarea
            ...
            <label style={{ display: 'grid', gap: '8px' }}>
              <span style={{ fontWeight: 600, color: sageGreenDark, fontSize: '15px' }}>Image URL (e.g., /images/plants/my-plant.jpg):</span>
              <input
                type="text"
                value={imageUrl}
                onChange={(e) => setImageUrl(e.target.value)}
                placeholder="/images/plants/my-plant.jpg"
                style={{
                  padding: '12px 16px',
                  borderRadius: borderRadiusSmall,
                  border: `2px solid ${warmCreamDark}`,
            ...
            >
              {formSubmitting ? 'Submitting...' : 'Add Plant'}
            </button>
            {formMessage && (
              <div style={{
                marginTop: '10px',
                padding: '12px',
                borderRadius: borderRadiusSmall,
                background: formMessage.includes('Success') ? '#D4EDDA' : '#F8D7DA',
                color: formMessage.includes('Success') ? '#155724' : '#721C24',
                border: `1px solid ${formMessage.includes('Success') ? '#C3E6CB' : '#F5C6CB'}`
              }}>
                {formMessage}
              </div>
            )}
