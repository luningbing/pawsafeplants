...
export async function getStaticProps() {
  ...
  // Filter: NOT Safe (includes Danger and Caution)
  const cautionPlants = plants.filter(p => {
    const L = String(p.toxicity_level || '').toLowerCase();
    return !L.includes('safe');
  });

  return { props: { plants: cautionPlants }, revalidate: 10 };
}

export default function CautionPlantsPage({ plants }) {
  ...
  return (
    <PageTransition>
      <Head>
        <title>Caution & Toxic Plants - PawSafe Plants</title>
        <meta name="description" content="Plants that require caution or are toxic to cats. Learn which ones to avoid." />
        <link rel="canonical" href="https://www.pawsafeplants.com/caution-plants" />
        <meta property="og:title" content="Caution & Toxic Plants - PawSafe Plants" />
        <meta property="og:description" content="Plants that require caution or are toxic to cats. Learn which ones to avoid." />
        <meta property="og:url" content="https://www.pawsafeplants.com/caution-plants" />
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ maxWidth: '800px', margin: '0 auto 40px auto', position: 'relative' }}>
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center', background: '#fff', borderRadius: '24px', boxShadow: '0 8px 24px rgba(0,0,0,0.1)', border: `2px solid ${warmCreamDark}`, overflow: 'hidden' }}>
              ...
              <input
                type="text"
                placeholder="Search caution/toxic plants..."
                value={searchQuery}
